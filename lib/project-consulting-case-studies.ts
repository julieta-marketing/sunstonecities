...
export interface ProjectConsultingCaseStudy {
  id: string
  slug: string
  title: string
  label: string
...
type ParsedProjectConsultingCaseStudy = ProjectConsultingCaseStudy & {
  sourceFile: string
}
...
function parseCaseStudyFile(
  filePath: string,
): ParsedProjectConsultingCaseStudy | null {
  ...
  if (!title || !label || !summary || !date || !challenge || !solution) {
    warnContentIssue(
      `Skipping Project Consulting case study with missing required fields: ${filePath}`,
    )
    return null
  }
  ...
  if (!slug) {
    warnContentIssue(
      `Skipping Project Consulting case study because a slug could not be generated: ${filePath}`,
    )
    return null
  }
...
export function getAllProjectConsultingCaseStudies() {
  ...
  const parsedCaseStudies = fs
    ...
    .filter(
      (
        caseStudy,
      ): caseStudy is ParsedProjectConsultingCaseStudy => Boolean(caseStudy),
    )
    .sort((a, b) => toDateValue(b.date) - toDateValue(a.date))

  const uniqueCaseStudies = new Map<string, ParsedProjectConsultingCaseStudy>()
  ...
  for (const caseStudy of parsedCaseStudies) {
    ...
    if (uniqueCaseStudies.has(slug)) {
      ...
      warnContentIssue(
        `Using generated Project Consulting case study slug "${slug}" for duplicate slug "${caseStudy.slug}": ${caseStudy.sourceFile}`,
      )
    }
...
export const projectConsultingCaseStudies = getAllProjectConsultingCaseStudies()

export const projectConsultingCaseStudySlides = projectConsultingCaseStudies.map(
  (caseStudy) => ({
    id: caseStudy.id,
...
export const featuredProjectConsultingCaseStudies: FeaturedCaseStudy[] =
  projectConsultingCaseStudies.map((caseStudy, index) => ({
    id: caseStudy.id,
    ...
    solution: caseStudy.solution,
    image: caseStudy.image,
    link: `/services/project-consulting#case-${caseStudy.id}`,
  }))

export function getFeaturedProjectConsultingCaseStudies(limit = 3) {
  return featuredProjectConsultingCaseStudies.slice(0, limit)
}
